...
namespace InventorWebViewer.Core
{
    ...
    {
        /// <summary>
        /// Writes a portable package:
        ///   index.html  — self-contained (GLB base64 inlined) when model fits in memory
        ///   scene.glb   — optional binary copy (desktop tools)
        ///   scene_b64.js — only for very large models that cannot be safely inlined
        /// Returns path of the single-file HTML when self-contained.
        /// </summary>
            ...
            byte[] sceneGlb = null)
        {
            ...
            // Prefer a single self-contained HTML for mobile/share.
            // Base64 grows ~33%; keep a safety ceiling to avoid OOM while building the string.
            const long InlineLimitBytes = 60L * 1024 * 1024; // 60 MB raw GLB → ~80 MB HTML
            bool canInline = sceneGlb != null && sceneGlb.Length > 64 && sceneGlb.Length <= InlineLimitBytes;
            bool hasExternalB64 = false;

            if (sceneGlb != null && sceneGlb.Length > 64 && !canInline)
            {
                // Very large models: external classic script (file:// safe)
                try
                {
            ...
            var html = BuildHtml(scene, settings, sceneJson, canInline ? sceneGlb : null, hasExternalB64, canInline);
            var htmlPath = IOPath.Combine(outDir, "index.html");
            IOFile.WriteAllText(htmlPath, html, new UTF8Encoding(false));

            // Also write a clearly named portable single file for mobile transfer
            if (canInline)
            {
                try
                {
                    var safe = SanitizeFileName(scene?.AssemblyName ?? "Viewer");
                    var portable = IOPath.Combine(outDir, safe + "_Portable.html");
                    IOFile.Copy(htmlPath, portable, true);
                }
                catch { /* optional */ }
            }
        ...
        {
            if (string.IsNullOrWhiteSpace(name)) return "Viewer";
            ...
            bool inlineGlb = false)
        {
            ...
            sb.AppendLine("<meta name=\"viewport\" content=\"width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover\"/>");
            ...
            sb.AppendLine("<meta name=\"apple-mobile-web-app-status-bar-style\" content=\"black-translucent\"/>");
            sb.AppendLine("<meta name=\"theme-color\" content=\"#0a0f1a\"/>");
            ...
            sb.AppendLine("  <button type=\"button\" id=\"btnMobileMenu\" class=\"mobile-fab mobile-only\" title=\"" + L("ابزارها", "Tools") + "\">☰</button>");
            sb.AppendLine("  <button type=\"button\" id=\"btnMobileMore\" class=\"mobile-fab mobile-only\" style=\"display:none;top:calc(66px + env(safe-area-inset-top));left:calc(10px + env(safe-area-inset-left))\" title=\"" + L("بیشتر", "More") + "\">···</button>");
            sb.AppendLine("  <div id=\"toolbar\" class=\"glass\">");
            sb.AppendLine("    <div class=\"tool-group tool-group-nav\">");
            sb.AppendLine("      <button type=\"button\" class=\"tool active\" data-tool=\"orbit\">⟳ <span class=\"tool-label\">" + L("چرخش", "Orbit") + "</span></button>");
            sb.AppendLine("      <button type=\"button\" class=\"tool\" data-tool=\"pan\">✥ <span class=\"tool-label\">" + L("جابجایی", "Pan") + "</span></button>");
            sb.AppendLine("      <button type=\"button\" class=\"tool\" data-tool=\"zoom\">⊕ <span class=\"tool-label\">" + L("زوم", "Zoom") + "</span></button>");
            sb.AppendLine("      <button type=\"button\" class=\"tool\" data-tool=\"fly\">✈ <span class=\"tool-label\">" + L("پرواز", "Fly") + "</span></button>");
            sb.AppendLine("      <button type=\"button\" class=\"tool\" data-tool=\"measure\">📏 <span class=\"tool-label\">" + L("اندازه", "Measure") + "</span></button>");
            sb.AppendLine("    </div>");
            sb.AppendLine("    <div class=\"tool-group tool-group-view\">");
            sb.AppendLine("      <button type=\"button\" class=\"tool\" id=\"btnFit\">⛶ <span class=\"tool-label\">" + L("فیت", "Fit") + "</span></button>");
            sb.AppendLine("      <button type=\"button\" class=\"tool\" id=\"btnHome\">⌂ <span class=\"tool-label\">" + L("خانه", "Home") + "</span></button>");
            ...
            sb.AppendLine("    </div>");
            sb.AppendLine("    <div class=\"tool-group tool-group-extra\">");
            ...
            sb.AppendLine("    </div>");
            sb.AppendLine("    <div class=\"tool-group tool-group-extra\">");
            sb.AppendLine("      <button type=\"button\" class=\"tool\" id=\"btnToggleTree\">☰ <span class=\"tool-label\">" + L("درخت", "Tree") + "</span></button>");
            sb.AppendLine("      <button type=\"button\" class=\"tool\" id=\"btnToggleGrid\"># <span class=\"tool-label\">" + L("شبکه", "Grid") + "</span></button>");
            sb.AppendLine("      <button type=\"button\" class=\"tool active\" id=\"btnToggleAxes\" title=\"" + L("محورهای XYZ", "XYZ axes") + "\">XYZ</button>");
            sb.AppendLine("      <button type=\"button\" class=\"tool\" id=\"btnSection\" title=\"" + L("صفحه برش", "Section plane") + "\">✂ <span class=\"tool-label\">" + L("برش", "Section") + "</span></button>");
            ...
            sb.AppendLine("  <aside id=\"sidebar\" class=\"glass draggable-panel collapsed\">");
            ...
            sb.AppendLine("        <button type=\"button\" id=\"btnCloseTree\" class=\"mini\" title=\"" + L("بستن", "Close") + "\">✕</button>");
            sb.AppendLine("        <button type=\"button\" id=\"btnPinSidebar\" class=\"mini pin-btn\" title=\"" + L("پین", "Pin") + "\">📌</button>");
            sb.AppendLine("      </div>");
            sb.AppendLine("    </div>");
            sb.AppendLine("    <div id=\"tree\"></div>");
            sb.AppendLine("  </aside>");
            sb.AppendLine("  <div id=\"sectionHud\" class=\"glass hidden\">");
            sb.AppendLine("    <label>✂ " + L("برش", "Section") + "</label>");
            sb.AppendLine("    <select id=\"sectionAxis\" class=\"tool-select\">");
            sb.AppendLine("      <option value=\"y\" selected>Y</option>");
            sb.AppendLine("      <option value=\"x\">X</option>");
            sb.AppendLine("      <option value=\"z\">Z</option>");
            sb.AppendLine("    </select>");
            sb.AppendLine("    <input type=\"range\" id=\"sectionSlider\" min=\"-100\" max=\"100\" value=\"0\" step=\"1\" />");
            sb.AppendLine("    <span id=\"sectionVal\">0</span>");
            sb.AppendLine("    <button type=\"button\" id=\"btnSectionFlip\" class=\"mini\" title=\"" + L("معکوس", "Flip") + "\">⇄</button>");
            sb.AppendLine("    <button type=\"button\" id=\"btnSectionClose\" class=\"mini\">✕</button>");
            sb.AppendLine("  </div>");
            ...
            // Geometry payload
            if (inlineGlb && sceneGlb != null && sceneGlb.Length > 64)
            {
                // Self-contained single HTML — works on desktop + mobile without extra files
                sb.AppendLine("<script>");
            ...
            else if (hasExternalB64)
            {
                // Very large models only
...
#viewport{position:absolute;inset:0;z-index:0;touch-action:none}
...
#sectionHud{
  ...
  z-index:28;display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:12px;
  font-size:12px;font-weight:600;flex-wrap:wrap;justify-content:center;
}
#sectionHud.hidden{display:none}
#sectionSlider{width:140px;accent-color:#ff8c5a;cursor:pointer}
#sectionVal{min-width:36px;text-align:right;font-variant-numeric:tabular-nums}
...
/* ========== Mobile / tablet ========== */
.mobile-fab{
  display:none;position:absolute;z-index:40;border:none;border-radius:14px;
  width:48px;height:48px;font-size:22px;font-weight:700;color:#eef6ff;
  background:rgba(12,18,32,0.88);backdrop-filter:blur(12px);
  box-shadow:0 6px 20px rgba(0,0,0,0.45);border:1px solid rgba(255,255,255,0.16);
  -webkit-tap-highlight-color:transparent;
}
#btnMobileMenu.mobile-fab{top:calc(10px + env(safe-area-inset-top));left:calc(10px + env(safe-area-inset-left))}

@media (max-width:900px){
  .mobile-only{display:flex !important;align-items:center;justify-content:center}
  #btnMobileMenu{display:flex !important}
  #toolbar{
    top:auto;bottom:calc(56px + env(safe-area-inset-bottom));left:8px;right:8px;
    transform:none;max-width:none;width:auto;
    padding:8px;gap:6px;border-radius:16px;
    max-height:42vh;overflow-y:auto;-webkit-overflow-scrolling:touch;
    display:none;flex-direction:column;align-items:stretch;
  }
  #toolbar.mobile-open{display:flex}
  #toolbar .tool-group{justify-content:flex-start}
  #toolbar .tool-group-extra{display:none}
  #toolbar.mobile-expanded .tool-group-extra{display:flex}
  .tool,.tool-select{
    min-height:42px;min-width:42px;padding:8px 12px;font-size:13px;
    touch-action:manipulation;
  }
  .tool-select{max-width:100%;flex:1}
  #toolbar .drag-handle{display:none}
  #sidebar{
    top:auto;left:0;right:0;bottom:0;width:100%;
    height:min(38vh,280px);max-height:38vh;
    border-radius:18px 18px 0 0;z-index:35;
    padding-bottom:calc(12px + env(safe-area-inset-bottom));
  }
  #sidebar.collapsed{display:none !important}
  #sidebar:not(.collapsed){display:flex}
  #sectionHud{
    top:auto;bottom:calc(8px + env(safe-area-inset-bottom));left:8px;right:8px;
    transform:none;flex-wrap:wrap;z-index:36;font-size:12px;padding:8px 10px;
  }
  #sectionSlider{width:min(140px,40vw);accent-color:#4fb3ff}
  #viewCube{
    top:calc(10px + env(safe-area-inset-top));right:calc(10px + env(safe-area-inset-right));
    width:72px;z-index:22;
  }
  #viewCube canvas{width:64px;height:64px}
  #viewCube .panel-chrome{display:none}
  #gridOffsetHud{
    bottom:calc(8px + env(safe-area-inset-bottom));left:8px;right:auto;
    padding:6px 10px;font-size:11px;z-index:24;
  }
  #gridOffsetSlider{width:90px}
  #measureHud,#selectHud{
    top:calc(64px + env(safe-area-inset-top));left:8px;right:8px;
    transform:none;flex-wrap:wrap;font-size:13px;padding:8px 10px;z-index:36;
  }
  #status{
    bottom:calc(8px + env(safe-area-inset-bottom));left:8px;right:8px;
    transform:none;max-width:none;font-size:11px;text-align:center;
  }
  #footer{
    display:none; /* free vertical space on phones */
  }
  #flyHud{
    bottom:calc(120px + env(safe-area-inset-bottom));left:8px;right:8px;transform:none;text-align:center;
  }
  .support-msg,.contacts{display:none}
  #loadingBarWrap{width:min(240px,70vw)}
  .side-header{gap:6px}
  .tree-item{padding:8px 6px;min-height:36px} /* larger hit targets */
}

@media (max-width:480px){
  .tool-label{display:none} /* icon-only buttons on very small screens */
  #viewCube{width:60px}
  #viewCube canvas{width:52px;height:52px}
  #toolbar{bottom:calc(48px + env(safe-area-inset-bottom));max-height:48vh}
}

@media (orientation:landscape) and (max-height:500px){
  #toolbar{max-height:70vh;bottom:calc(8px + env(safe-area-inset-bottom))}
  #sidebar{height:min(42vh,220px);max-height:42vh}
  #viewCube{width:56px}
  #viewCube canvas{width:48px;height:48px}
  #footer{display:none}
}

@media (hover:none) and (pointer:coarse){
  /* Touch devices: larger controls even on tablets in landscape */
  .tool,.mini{min-height:40px}
  #c{touch-action:none}
}
";
        }
...
let sectionHelper = null;   // visible plane mesh with hatch pattern
let sectionAxis = 'y';
let sectionOffset = 0;      // -1..1
let sectionFlip = 1;        // 1 or -1
...
function init() {
  ...
  renderer = new THREE.WebGLRenderer({ canvas: canvas, antialias: !isMobileUi(), alpha: true, powerPreference: 'high-performance' });
  // Cap DPR lower on phones to keep FPS stable
  var maxDpr = isMobileUi() ? 1.25 : 1.5;
  ...
  // Slightly slower rotate on touch for stability
  try {
    controls.rotateSpeed = isMobileUi() ? 0.75 : 1.0;
    controls.zoomSpeed = isMobileUi() ? 0.9 : 1.0;
    controls.panSpeed = isMobileUi() ? 0.7 : 1.0;
  } catch (eTouch) {}
  ...
  $('#btnToggleTree').addEventListener('click', function() {
    sidebar.classList.toggle('collapsed');
    if (isMobileUi() && !sidebar.classList.contains('collapsed')) {
      // close tools sheet when opening tree on mobile
      var tb = $('#toolbar'); if (tb) tb.classList.remove('mobile-open');
    }
  });
  initMobileChrome();
  ...
  if (btnSection) btnSection.addEventListener('click', function() {
    ...
    var sh = $('#sectionHud');
    if (sh) sh.classList.toggle('hidden', !sectionEnabled);
    applySectionClipping();
  });
  var sectionSlider = $('#sectionSlider');
  if (sectionSlider) sectionSlider.addEventListener('input', function() {
    sectionOffset = parseFloat(sectionSlider.value) / 100;
    applySectionClipping();
  });
  var sectionAxisSel = $('#sectionAxis');
  if (sectionAxisSel) sectionAxisSel.addEventListener('change', function(e) {
    sectionAxis = e.target.value || 'y';
    applySectionClipping();
  });
  var btnFlip = $('#btnSectionFlip');
  if (btnFlip) btnFlip.addEventListener('click', function() {
    sectionFlip *= -1;
    applySectionClipping();
  });
  var btnSecClose = $('#btnSectionClose');
  if (btnSecClose) btnSecClose.addEventListener('click', function() {
    sectionEnabled = false;
    if (btnSection) btnSection.classList.remove('active');
    var sh = $('#sectionHud'); if (sh) sh.classList.add('hidden');
    applySectionClipping();
  });
  var btnCloseTree = $('#btnCloseTree');
  if (btnCloseTree) btnCloseTree.addEventListener('click', function() {
...
// ---- Section plane clipping + hatch helper ----
...
function ensureSectionHelper() {
  if (sectionHelper) return;
  var size = Math.max(sceneMaxDim * 2.2, 20);
  var geo = new THREE.PlaneGeometry(size, size, 1, 1);
  var mat = new THREE.MeshBasicMaterial({
    map: makeHatchTexture(),
    transparent: true,
    opacity: 0.55,
...
function applySectionClipping() {
  ...
  else if (sectionAxis === 'z') n.set(0, 0, -1);
  else n.set(0, -1, 0);
  n.multiplyScalar(sectionFlip);

  var offset = sectionOffset * sceneMaxDim;
  var point = sceneCenter.clone().addScaledVector(n, -offset);
  // Plane: normal·x + constant = 0  → constant = -normal·point
  sectionPlane.normal.copy(n);
  sectionPlane.constant = -n.dot(point);

  var valEl = $('#sectionVal');
  ...
  if (sectionHelper) {
    sectionHelper.visible = sectionEnabled;
    if (sectionEnabled) {
      var size = Math.max(sceneMaxDim * 2.2, 20);
      sectionHelper.scale.setScalar(size / Math.max(sceneMaxDim * 2.2, 20));
      // Orient plane to match normal
      sectionHelper.position.copy(point);
      sectionHelper.lookAt(point.clone().add(n));
    }
  }
}
...
function makeDraggable(el, handle) {
  if (!el || !handle) return;
  // Skip drag on coarse-pointer phones — panels use fixed mobile layout
...
function isMobileUi() {
  try {
    return window.matchMedia('(max-width:900px)').matches ||
      (window.matchMedia('(hover:none) and (pointer:coarse)').matches && window.innerWidth < 1100);
  } catch (e) {
...
function initMobileChrome() {
  var menuBtn = $('#btnMobileMenu');
  var moreBtn = $('#btnMobileMore');
  var tb = $('#toolbar');
  if (!menuBtn || !tb) return;

  menuBtn.addEventListener('click', function(e) {
    e.stopPropagation();
    var open = tb.classList.toggle('mobile-open');
    menuBtn.textContent = open ? '✕' : '☰';
    if (moreBtn) moreBtn.style.display = open && isMobileUi() ? 'flex' : 'none';
    if (!open) tb.classList.remove('mobile-expanded');
  });

  if (moreBtn) {
    moreBtn.addEventListener('click', function(e) {
      e.stopPropagation();
      tb.classList.toggle('mobile-expanded');
      moreBtn.textContent = tb.classList.contains('mobile-expanded') ? '–' : '···';
    });
  }

  // Tap empty canvas area closes mobile tool sheet (not while measuring)
  var canvas = $('#c');
  if (canvas) {
    canvas.addEventListener('pointerdown', function() {
      if (!isMobileUi() || !tb.classList.contains('mobile-open')) return;
      // delay so tool buttons still receive their click
      setTimeout(function() {
        if (currentTool === 'measure') return;
        tb.classList.remove('mobile-open');
        tb.classList.remove('mobile-expanded');
        menuBtn.textContent = '☰';
  ...
  // Tree: collapsed by default on mobile; open on desktop
  if (sidebar) {
    if (isMobileUi()) sidebar.classList.add('collapsed');
  ...
  window.addEventListener('orientationchange', function() {
    setTimeout(function() {
      onResize();
      if (sidebar && isMobileUi()) sidebar.classList.add('collapsed');
    }, 200);
  });
}
");
