...
namespace InventorWebViewer
{
    ...
    public class StandardAddInServer : ApplicationAddInServer
    {
        ...
        private void ExportButton_OnExecute(NameValueMap context)
        {
            ...
            bool isAssembly = active != null && active.DocumentType == DocumentTypeEnum.kAssemblyDocumentObject;
            bool isPart = active != null && active.DocumentType == DocumentTypeEnum.kPartDocumentObject;
            if (active == null || (!isAssembly && !isPart))
            {
            ...
            try
            {
                using (var mainForm = new MainForm(_invApp, (Document)active))
                    mainForm.ShowDialog();
            }
            catch (Exception ex)
            {
